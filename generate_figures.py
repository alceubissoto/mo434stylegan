...
def load_Gs_fromdir(resume_run_id, resume_snapshot):
...
#---------------------------------------------------------------------------
# Generate images and save to folder
def generate_and_save(png, Gs, seed, amount, label):
    latents = np.random.RandomState(seed).randn(amount, Gs.input_shape[1])
    ...
    images = Gs.run(latents, label_array, **synthesis_kwargs)
    image_iter = iter(list(images))
    for i in range(amount):
        tmp = next(image_iter)
        print(tmp.shape)
        image = PIL.Image.fromarray(tmp[:,:,:3], 'RGB')
        image.save(png + '/00006-sgan-concat-1gpu-cond-grpc/images_7740/' + diag + '_'+str(i).zfill(5)+'.png')
        #image.save(png + '/00005-sgan-skin-1gpu-cond-grpc/images_dummy/' + diag + '_'+str(i).zfill(5)+'.png')
        attr0 = PIL.Image.fromarray(tmp[:,:,3], 'L')
        attr0.save(png + '/00006-sgan-concat-1gpu-cond-grpc/images_7740/' + diag+ '_'+str(i).zfill(5)+'_attr0.png')
        attr1 = PIL.Image.fromarray(tmp[:,:,4], 'L')
        attr1.save(png + '/00006-sgan-concat-1gpu-cond-grpc/images_7740/' + diag+ '_'+str(i).zfill(5)+'_attr1.png')
        attr2 = PIL.Image.fromarray(tmp[:,:,5], 'L')
        attr2.save(png + '/00006-sgan-concat-1gpu-cond-grpc/images_7740/' + diag+ '_'+str(i).zfill(5)+'_attr2.png')
        attr3 = PIL.Image.fromarray(tmp[:,:,6], 'L')
        attr3.save(png + '/00006-sgan-concat-1gpu-cond-grpc/images_7740/' + diag+ '_'+str(i).zfill(5)+'_attr3.png')
        attr4 = PIL.Image.fromarray(tmp[:,:,7], 'L')
        attr4.save(png + '/00006-sgan-concat-1gpu-cond-grpc/images_7740/' + diag+ '_'+str(i).zfill(5)+'_attr4.png')
...
def main():
    ...
    #generate_and_save(config.result_dir, load_Gs_fromdir(5, 7740), 0, 1000, [1,0])
    generate_and_save(config.result_dir, load_Gs_fromdir(6, 7740), 0, 10, [1,0])
    #generate_and_save(config.result_dir, load_Gs_fromdir(5, 7740), 0, 100, None)
