...
def training_loop(
    ...
    misc.save_image_grid(grid_reals[:,:3,:,:], os.path.join(submit_config.run_dir, 'reals.png'), drange=training_set.dynamic_range, grid_size=grid_size)
    misc.save_image_grid(grid_reals[:,3,:,:], os.path.join(submit_config.run_dir, 'real_attr.png'), drange=training_set.dynamic_range, grid_size=grid_size)
    misc.save_image_grid(grid_fakes[:,:3,:,:], os.path.join(submit_config.run_dir, 'fakes-%06d.png' % resume_kimg), drange=drange_net, grid_size=grid_size)
    misc.save_image_grid(grid_fakes[:,3,:,:], os.path.join(submit_config.run_dir, 'fakes_attr0-%06d.png' % resume_kimg), drange=drange_net, grid_size=grid_size)
    misc.save_image_grid(grid_fakes[:,4,:,:], os.path.join(submit_config.run_dir, 'fakes_attr1-%06d.png' % resume_kimg), drange=drange_net, grid_size=grid_size)
    misc.save_image_grid(grid_fakes[:,5,:,:], os.path.join(submit_config.run_dir, 'fakes_attr2-%06d.png' % resume_kimg), drange=drange_net, grid_size=grid_size)
    ...
    while cur_nimg < total_kimg * 1000:
        ...
        if cur_nimg >= tick_start_nimg + sched.tick_kimg * 1000 or done:
            ...
            if cur_tick % image_snapshot_ticks == 0 or done:
                grid_fakes = Gs.run(grid_latents, grid_labels, is_validation=True, minibatch_size=sched.minibatch//submit_config.num_gpus)
                misc.save_image_grid(grid_fakes[:,:3,:,:], os.path.join(submit_config.run_dir, 'fakes-%06d.png' % (cur_nimg // 1000)), drange=drange_net, grid_size=grid_size)
                misc.save_image_grid(grid_fakes[:,3,:,:], os.path.join(submit_config.run_dir, 'fakes_attr0-%06d.png' % (cur_nimg // 1000)), drange=drange_net, grid_size=grid_size)
                misc.save_image_grid(grid_fakes[:,4,:,:], os.path.join(submit_config.run_dir, 'fakes_attr1-%06d.png' % (cur_nimg // 1000)), drange=drange_net, grid_size=grid_size)
                misc.save_image_grid(grid_fakes[:,5,:,:], os.path.join(submit_config.run_dir, 'fakes_attr2-%06d.png' % (cur_nimg // 1000)), drange=drange_net, grid_size=grid_size)
