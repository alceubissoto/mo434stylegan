...
def parse_tfrecord_tf(record):
    features = tf.parse_single_example(record, features={
        ...
        #'shape': tf.FixedLenFeature([8], tf.int64),
...
class TFRecordDataset:
    def __init__(self,
        ...
        if self.label_file is not None and max_label_size != 0:
            ...
            onehot = []
            for label in self._np_labels:
                tmp = np.array([0., 0.], dtype=np.float32)
                tmp[int(label)] = 1.
                onehot.append(tmp)
            onehot = np.array(onehot, dtype=np.float32)
            self._np_labels = onehot
            print(self._np_labels, self._np_labels.shape)
...
class SyntheticDataset:
    def __init__(self, resolution=1024, num_channels=8, dtype='uint8', dynamic_range=[0,255], label_size=0, label_dtype='float32'):
