...
def create_from_images(tfrecord_dir, image_dir, shuffle):
    ...
    f = open('/work/isic2018-train-512/isic2018-part2-all.csv', 'r')
    labels = {}
    f.readline()
    for line in f:
        labels[line.split(';')[0]] = int(line.split(';')[1].strip())
    sorted_keys = sorted(labels.keys())

    sorted_values = [labels[key] for key in sorted_keys]
    ...
    with TFRecordExporter(tfrecord_dir, len(image_filenames)) as tfr:
        ...
        for idx in range(order.size):
            ...
            img = img.transpose([2, 0, 1]) # HWC => CHW
            tfr.add_image(img)
        tfr.add_labels(sorted_values[order])        
